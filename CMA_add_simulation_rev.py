...
for a in itertools.product([0, 1], repeat = bit_number):
  for b in itertools.product([0, 1], repeat = bit_number):
    ...
    carry_sub = 0
    for j in range(bit_number):
        bit1_sub = result[bit_number - 1 -j]   
        bit2_sub = minas_b[bit_number -1 -j]    
    ...
    if list(a) == result_a:
      ...
      # 復号できた入力パターンとcountを記憶
      dec_count_list.append(count)
      # aとbをそれぞれjoinしてint型に変換し，10進数に変換，リストに格納
      x.append(int("".join(map(str, a)), 2))
      y.append(int("".join(map(str, b)), 2))

print(f"復号可能回数:      {dec_count}")

# グラフ描画
plt.xlabel("a(decimal)")
plt.ylabel("b(decimal)")

# タイトルを表示
plt.title("Decryptable patterns")

# 散布図を描画
plt.scatter(x, y, s=5, c='blue', marker='o', label='decodable')
